...
class StoreDashboardController extends Controller
{
    ...
    public function index(){
		...
		$newOrder = Payment::whereIn('carts_id',$cartIds)->where('order_status', '1')->count();
		...
		$totalOrders = Payment::whereIn('carts_id',$cartIds)->where('order_status', '2')->count();
		...
		$products = Products::where('store_id',\Auth::id())->where('qty', '<=', @$settings->low_quantity)->count();
		...
		$dailyCancleOrder = Payment::whereIn('carts_id',$cartIds)
		    ->where('order_status', '3')
			->where('created_at', '>=', $cdate)
			->count();
		
		$dailyOrder = Payment::whereIn('carts_id',$cartIds)
		    ->where('order_status', '2')
			->where('created_at', '>=', $cdate)
			->count();			
		
		$dailySales = Payment::whereIn('carts_id',$cartIds)
		...
		// $dailyUser = User::whereIn('carts_id',$cartIds)->where('role', 'user')
		// 	->where('created_at', '>=', $cdate)
		// 	->count();
		
 
		return view('store.dashboard.index', compact('title', 'newOrder', 'dailyOrder', 'totalOrders', 'dailyCancleOrder', 'sales', 'dailySales', 'products', 'settings'));
	}
}
